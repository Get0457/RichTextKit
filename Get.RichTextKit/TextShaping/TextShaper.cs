...
namespace Get.RichTextKit
{
    ...
    internal class TextShaper : IDisposable
    {
        ...
        {
            if (style is ICustomRenderSizeStyle customStyle)
            {
                return ShapeCustomSize(bufferSet, codePoints, customStyle, clusterAdjustment, direction);
            }
            ...
            //if (style is ICustomRenderSizeStyle cs)
            //    forceFixedPitchWidth = cs.CustomRenderSize.Width;
            //else
        ...
        private Result ShapeCustomSize(ResultBufferSet bufferSet, Slice<int> codePoints, ICustomRenderSizeStyle style, int clusterAdjustment, TextDirection direction)
        {
            using (var buffer = new HarfBuzzSharp.Buffer())
            {
                // Setup buffer
                ...
                var size = style.CustomRenderSize;
                //r.Ascent = size.Height - r.Descent;
                var newAscent = r.Ascent;
                var newDescent = size.Height + r.Ascent;
                switch (style.TextVerticalAlignment)
                {
                    case TextVerticalAlignment.Top:
                        break;
                    case TextVerticalAlignment.Center:
                        newAscent += size.Height / 2;
                        newDescent += size.Height / 2;
                        break;
                    case TextVerticalAlignment.Bottom:
                        newAscent += size.Height;
                        newDescent += size.Height;
                        break;
                }
                r.Ascent = newAscent;
                r.Descent = newDescent;
                r.EndXCoord = new SKPoint(size.Width, 0);
                r.XMin = size.Width;
                return r;
            }
        }

    ...
    public class FontRun
    {
        ...
        internal void Paint(PaintTextContext ctx)
        {
            ...
            if (ctx.PaintSelectionBackground != null && RunKind != FontRunKind.Ellipsis)
            {
                ...
                if (Direction is TextDirection.LTR && ctx.SelectionStart > End || ctx.SelectionEnd < Start)
                    goto AfterPaintingSelection;
        ...
        AfterPaintingSelection:
            ...
            using (var paintHalo = new SKPaint())
            {
                ...
                // Setup SKPaint
                paint.Color = Style.TextColor ?? ctx.Options.TextDefaultColor;
            ...
            if (Style is INotifyRenderStyle customStyle)
                customStyle.OnRender(new SKRect(
                    ctx.Canvas.TotalMatrix.TransX + XCoord,
                    ctx.Canvas.TotalMatrix.TransY + Line.YCoord + Line.BaseLine + Ascent,
                    ctx.Canvas.TotalMatrix.TransX + XCoord + Width,
                    ctx.Canvas.TotalMatrix.TransY + Line.YCoord + Line.BaseLine + Descent
                ));
        }

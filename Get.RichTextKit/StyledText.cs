...
namespace Get.RichTextKit
{
    ...
    public class StyledText
    {
        ...
        public void AddText(Slice<int> text, IStyle style)
        {
            ...
            if (style is INotifyInvalidateStyle nis)
            {
                nis.Invalidate += OnChanged;
            }
        ...
        public void ApplyStyle(int position, int length, IStyle style)
        {
            ...
            if (position == 0 && length == this.Length)
            {
                int i = 0;
                // Remove excess runs
                while (_styleRuns.Count > i + 1)
                {
                    if (_styleRuns[i + 1].Style is IDoNotCombineStyle)
                    {
                        var oldStart = _styleRuns[i].Start;
                        _styleRuns[i].Start = position;
                        _styleRuns[i].Length = oldStart - position;
                        _styleRuns[i].Style = style;
                        i++;
                        // do not modify this styleRun position and length
                        if (_styleRuns[i].Style is IWriteableStyle ws)
                            CopyStyle.Copy(style, ws);
                        position = _styleRuns[i].End;
                        i++;
                    } else
                    {
                        StyleRun.Pool.Value.Return(_styleRuns[i + 1]);
                        _styleRuns.RemoveAt(i + 1);
                    }
                }

                // Reconfigure the first
                _styleRuns[i].Start = position;
                _styleRuns[i].Length = length - position;
                if (_styleRuns[i] is IDoNotCombineStyle)
                {
                    if (_styleRuns[i] is IWriteableStyle ws)
                    {
                        CopyStyle.Copy(style, ws);
                    }
                }
            ...
            foreach (var subRun in _styleRuns.GetIntersectingRunsReverse(position, length))
            {
                ...
                {
                    if (_styleRuns[subRun.Index].Style is IDoNotCombineStyle)
                    {
                        if (_styleRuns[subRun.Index].Style is IWriteableStyle ws)
                        {
                            CopyStyle.Copy(style, ws);
                        }
                        goto AfterInsert;
            ...
            if (newRun.Style is IDoNotCombineStyle)
            {
                if (newRun.Style is IWriteableStyle ws)
                {
                    CopyStyle.Copy(style, ws);
                }
            }
        ...
        void CoalescStyleRuns()
        {
            ...
            for (int i = 1; i < _styleRuns.Count; i++)
            {
                ...
                if (!(run.Style is IDoNotCombineStyle) && run.Style == prev.Style)
                {
                    // Yes
